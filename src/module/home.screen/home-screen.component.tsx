...
export function HomeScreen({navigation} : RootNavigationProps<'Home'>) {
  ...
  const [pruducts, setProducts] = useState<IProductM[]>([
    {
  ...
  useEffect(() => {
    setPaintingSupplies(
      pruducts.filter(pruduct => pruduct.categories === 'Малярные товары'),
    );
    setOveralls(
      pruducts.filter(pruduct => pruduct.categories === 'Спецодежда'),
    );
    setForHomeAndCottage(
      pruducts.filter(pruduct => pruduct.categories === 'Для дома и дачи'),
    );
    setElectricals(
      pruducts.filter(pruduct => pruduct.categories === 'Электрооборудование'),
    );
}, []);
  ...
  return (
    <View style={styles.homeScreen}>
      ...
      <ScrollView style={styles.scrollWiew}>
        ...
        <View style={styles.cards}>
          {overalls.map(overal => (
            <CardItem navigation={navigation} key={overal.id} product={overal} />
          ))}
        </View>
        <Text style={styles.title}>Для дома и дачи</Text>
        <View style={styles.cards}>
          {forHomeAndCottage.map(product => (
            <CardItem navigation={navigation} key={product.id} product={product} />
          ))}
        </View>
        <Text style={styles.title}>Электрооборудование</Text>
        <View style={styles.cards}>
          {electricals.map(electrical => (
            <CardItem navigation={navigation} key={electrical.id} product={electrical} />
          ))}
        </View>
      </ScrollView>
    </View>
